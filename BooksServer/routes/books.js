...
router.put('/updatebook/:id',(req,res)=>{
    Book.findOneAndUpdate({_id:req.params.id},{$set:{title: req.body.title,author: req.body.author,
        category: req.body.category}},(err,book)=>{
        if(err){
...
router.put('/rentbook/:id',(req,res)=>{
    Book.findOneAndUpdate({_id:req.params.id},{$set:{isRented: req.body.isRented}},(err,book)=>{
        if(err){
            res.send("error in updating book")
        }
        else{
